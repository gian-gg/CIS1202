...
$fuel = isset($_POST["fuelInput"]) ? $_POST["fuelInput"] : null;

function calculateFuelEfficiency($distance, $fuel)
{
  // Avoid division by zero
  if ($fuel <= 0) {
    return "Invalid input: Fuel consumption must be greater than zero.";
  }

  $efficiency = $distance / $fuel;

  return number_format($efficiency, 2) . " km per liter";
}
  ...
  <main
    ...
    <div
      ...
      <div class="card-header">
        ...
        <h1 class="custom-text-primary m-0">Aviation:</h1>
        <p>Aircraft Fuel Efficiency Calculator.</p>
        <hr />
      </div>
      <div class="card-body d-flex flex-column gap-1 py-0">
        <form method="post">
          <div class="mb-3">
            <label for="distanceInput" class="form-label">Flight Distance (km)</label>
            <input
              type="number"
          ...
          <div class="mb-3">
            <label for="fuelInput" class="form-label">Fuel Consumption (L)</label>
            <input
              type="number"
              class="form-control border border-secondary text-light"
        ...
        <div class="text-white mt-4 d-flex gap-2 justify-content-center">
          <?php
          if ($distance && $fuel) {
            echo "<h5>Fuel Efficiency:</h5>";
            echo "<p>" . calculateFuelEfficiency($distance, $fuel) . "</p>";
          }
          ?>
        </div>
      </div>
